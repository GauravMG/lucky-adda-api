...
class CommonModel {
    ...
    ID_COLUMN_NAME;
    SEARCH_COLUMN_NAME;
    constructor(tableName, idColumnName, searchColumnName) {
        this.TABLE_NAME = tableName;
        this.ID_COLUMN_NAME = idColumnName;
        this.SEARCH_COLUMN_NAME = searchColumnName;
    }
    rawQuery = async (transaction, query) => {
        try {
            // Start a transaction
    ...
    bulkCreate = async (transaction, inputData, createdById) => {
        try {
            ...
            for (let i = 0; i < inputData.length; i++) {
                if (Object.keys(inputData[i]).length) {
                    ...
                    for (const el of elements) {
                        ...
                        inputData[i][el] = await (0, helpers_1.escapeJSONString)(inputData[i][el]);
                        // Remove falsy or empty values, or trim strings
                        if ((["boolean", "number"].indexOf(typeof inputData[i][el]) < 0 &&
                            !inputData[i][el]) ||
    ...
    list = async (transaction, { filter = {}, filtersNotBeIncluded = {}, customFilters = [], range, sort = [{ orderBy: "createdAt", orderDir: "desc" }], fields = [], isCountOnly = false, isIncludeDeleted = false, groupBy = [] }) => {
        try {
            // Dynamically get the model from Prisma Client
            const model = transaction[this.TABLE_NAME];
            // Ensure the model exists in Prisma Client
            ...
            const where = isIncludeDeleted ? {} : { deletedAt: null }; // Exclude soft-deleted entries by default
            ...
            const page = range?.page || common_1.DEFAULT_PAGE;
            const pageSize = range?.pageSize || common_1.DEFAULT_PAGE_SIZE;
            const skip = (page - 1) * pageSize;
            const take = pageSize;
            // Default sorting
            const finalSort = sort?.map((s) => ({
    ...
    updateById = async (transaction, data, id, updatedById) => {
        try {
            // Dynamically get the model from Prisma Client
            const model = transaction[this.TABLE_NAME];
            // Ensure the model exists in Prisma Client
            ...
            const updateData = {}; // To hold the fields to update
            const columns = Object.keys(data);
            // Process each column in the data object
            for (let column of columns) {
                // Escape JSON formatted data if necessary
                data[column] = await (0, helpers_1.escapeJSONString)(data[column]);
    ...
    updateByFilters = async (transaction, data, filter, updatedById) => {
        try {
            // Dynamically get the model from Prisma Client
            const model = transaction[this.TABLE_NAME];
            // Ensure the model exists in Prisma Client
    ...
    softDeleteByIds = async (transaction, ids, deletedById) => {
        try {
            // Dynamically get the model from Prisma Client
            const model = transaction[this.TABLE_NAME];
            // Ensure the model exists in Prisma Client
    ...
    softDeleteByFilter = async (transaction, filter, deletedById) => {
        try {
            // Dynamically get the model from Prisma Client
            const model = transaction[this.TABLE_NAME];
            // Ensure the model exists in Prisma Client
