...
class WalletController {
    ...
    async create(req, res, next) {
        try {
            ...
            const [wallet] = await PrismaLib_1.prisma.$transaction(async (transaction) => {
                let wallet = await this.commonModelWallet.bulkCreate(transaction, payload.map((el) => ({
                    ...el,
                    approvalStatus: el.transactionType === "credit" ? "approved" : "pending",
                ...
                if (newPayload.length) {
                    const walletBonus = await this.commonModelWallet.bulkCreate(transaction, newPayload, userId);
    ...
    async list(req, res, next) {
        try {
            ...
            const [wallets, total] = await PrismaLib_1.prisma.$transaction(async (transaction) => {
                ...
                const userToUserIdMap = new Map(users.map((user) => [user.userId, user]));
                const gameToGameIdMap = new Map(games.map((game) => [game.gameId, game]));
                wallets = wallets.map((wallet) => {
                    const user = userToUserIdMap.get(wallet.userId);
                    let userBetIdSet = null;
                    if ((wallet.userBetIds ?? "").trim() !== "") {
                        userBetIdSet = new Set(wallet.userBetIds.split(",").map((el) => Number(el)));
                    }
                    return {
                        ...wallet,
                        user,
                        game: (wallet.gameId ?? "").toString().trim() !== ""
    ...
    async update(req, res, next) {
        try {
            ...
            const [walletTransaction] = await PrismaLib_1.prisma.$transaction(async (transaction) => {
                // check if transaction exists
                const [existingTransaction] = await this.commonModelWallet.list(transaction, {
                    filter: {
                        walletId
                    }
                });
                ...
                if (restPayload?.approvalStatus === "approved") {
                    ...
                    if (Number(totalWinningBalance) < Number(existingTransaction.amount)) {
                        throw new exceptions_1.BadRequestException("Cannot approve withdraw request as winning balance is low.");
                    }
                }
                // update transaction
                await this.commonModelWallet.updateById(transaction, restPayload, walletId, userId);
                ...
                // get updated details
                const [walletTransaction] = await this.commonModelWallet.list(transaction, {
                    filter: {
                        walletId
                    }
                });
    ...
    async topWinner(req, res, next) {
        try {
            ...
            const [data] = await PrismaLib_1.prisma.$transaction(async (transaction) => {
                ...
                // fetch mapping data
                const userIds = wallets.map(({ userId }) => userId);
                ...
                wallets?.map((wallet) => {
                    ...
                    const userWalletIndex = combinedWalletUserIds.indexOf(wallet.userId);
                    if (wallet.transactionType === "credit" &&
                        wallet.approvalStatus === "approved") {
                        combinedWallets[userWalletIndex].totalWinnings += Number(wallet.amount);
                    }
                });
                return [
                    combinedWallets
    ...
    async convertWinning(req, res, next) {
        try {
            const response = new APIResponse_1.ApiResponse(res);
            const { userId, roleId } = req.headers;
            ...
            await PrismaLib_1.prisma.$transaction(async (transaction) => {
                ...
                overallWinnings.map((overallWinning) => (totalWinnings += Number(overallWinning.amount)));
                conversionDebits.map((conversionDebit) => (totalConversions += Number(conversionDebit.amount)));
                const balanceWinnings = totalWinnings - totalConversions;
                if (balanceWinnings < amount) {
                    throw new exceptions_1.BadRequestException(`Not enough winning amount balance to convert. Winning amount balance = ₹ ${balanceWinnings}`);
                }
                const [conversionDebit] = await this.commonModelWallet.bulkCreate(transaction, [
                    {
                        userId,
                        transactionType: "debit",
                        amount,
                ...
                const [conversionCredit] = await this.commonModelWallet.bulkCreate(transaction, [
                    {
                        userId,
                        transactionType: "credit",
                        amount,
                ...
                const [conversionBonus] = await this.commonModelWallet.bulkCreate(transaction, [
                    {
                        userId,
                        transactionType: "credit",
                        amount: (amount *
